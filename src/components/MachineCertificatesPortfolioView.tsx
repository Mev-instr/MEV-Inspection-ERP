...
export function MachineCertificatesPortfolioView({ employees, customers = staticCustomers, certificates, onCertificatesChange, inspectionReports = [], machineDetails = [], onUploadImage, allCertificateIds = [] }: MachineCertificatesPortfolioProps) {
  ...
  const handleImportCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const smartMap = (header: string): string => {
      ...
      if (h === "expiration date" || h === "expiry_date") return "expirationDate";
      if (h === "type of inspection" || h === "inspection_type") return "typeOfInspection";
      if (h === "next inspection date" || h === "next_inspection") return "nextInspectionDate";
  ...
  const handleExecuteImport = () => {
    ...
    const importedCertificates: MachineCertificate[] = [];
    ...
    allRows.forEach((row) => {
      ...
      const expirationDate = getVal("expirationDate") || new Date().toISOString().split("T")[0];
              ...
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-xl font-bold text-slate-800 font-sans flex items-center gap-2">
                    ...
                    Map CSV fields to machine certificates
                  </h3>
                  <p className="text-xs text-slate-500 mt-1">
                    Select fields from your CSV file to map against machine certificate fields, or to ignore during import.
                  </p>
                </div>
                <button
                        ...
                        <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-5 py-4">
                            ...
                            >
                              ...
                              <option value="equipmentName">Equipment / Machine Name</option>
                              ...
                              <option value="expirationDate">Expiration Date (Expiry)</option>
                              <option value="typeOfInspection">Type of Inspection</option>
                              <option value="nextInspectionDate">Next Inspection Date</option>
